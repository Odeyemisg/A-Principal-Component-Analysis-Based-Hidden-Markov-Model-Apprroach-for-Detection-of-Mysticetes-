...
Coeff=12; %number of filter coefficient
 ...
 for  ii =1: 1%size(D,2)
      XX=cell2mat(D(ii));
      KK=mfcc(XX,fs_1,'WindowLength',o_v,'OverlapLength',0,'NumCoeffs',Coeff-1);  % Extracting features using MFCC
     Data{ii}=KK;
 end
 Data;
...
%%%%%%Detecting%%%%%%%
 Test_file=mfcc( X_DATA,fs_1,'WindowLength',o_v,'OverlapLength',0,'NumCoeffs',Coeff-1);%Extracting feature from unknown sound file
