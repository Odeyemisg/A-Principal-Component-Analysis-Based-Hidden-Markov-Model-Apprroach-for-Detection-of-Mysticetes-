...
clear all
 ...
 dt=1/fs_1;
 div=10; % number of values used to split a frame to for a matrix
 r=div; %The length of the row of X_1 or X_2  iff its column length is longer than the row length. That is div=r
 %%%%%%%%%%HMM_DATA_TRAIN%%%%%%%%%%%
 ...
 for  ii =1:size(D,2)
      ...
      KK=[];
      u=0;
     for jj=t:t:length(XX)
         y=XX(1+u:jj);
         %%Create X_1 and X_2 and using DMD
       X_X=reshape(y,t/div,div);
       X_1=X_X(:,1:end-1);
       X_2=X_X(:,2:end);
          omega = DMD(X_1,X_2,r,dt); %Extracting features using DMD
           FE=abs(omega);
           FE=FE';
           KK=[KK;FE];
     u=u+t;
...
%%%%%%%%HMM_NOISE_TRAIN%%%%%%%%%
 ...
 for  ii =1:size(D,2)
     ...
     for jj=t:t:length(XX)
         y=XX(1+u:jj);
         %Create X_1 and X_2 and using DMD
       X_X=reshape(y,div,t/div);
       X_1=X_X(:,1:end-1);
       X_2=X_X(:,2:end);
         FE = DMD(X_1,X_2,r,dt); % Extracting features using DMD
        omega = DMD(X_1,X_2,r,dt);
           FE=abs(omega);
           FE=FE';
           KK=[KK;FE];
     u=u+t;
...
%%%%%%Detecting%%%%%%%
  Test_file=Extract_dmd(X_DATA,t,dt,r,div); %Extracting feature from unknown sound file
...
Recog=Recognise(path_data,t,state_num,syn);
