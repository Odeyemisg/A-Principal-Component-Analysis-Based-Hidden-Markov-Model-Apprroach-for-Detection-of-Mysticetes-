...
clear all
 ...
 for  ii =1: size(D,2)
     ...
     [VV, GGG]=emd(XX,'MaxNumIMF', 6); %Extracting features using EMD
     [YUPPER,YLOWER]= envelope(VV,2,'rms');
     VV=YUPPER-YLOWER;
       KK=[];
      u=0;
     for aa=t:t:size(VV,1)
        y=VV(1+u:aa,:);
     y=mean(y);
     KK=[KK;y];
     u=u+t;   
...
%%%%%%Detecting%%%%%%%
 Test_file=Extract_emd(X_DATA,t); %Extracting feature from unknown sound file
